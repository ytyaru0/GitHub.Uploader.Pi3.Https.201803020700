...
class SettingMeta(type):
    def __new__(cls, name, bases, attrs):
        ...
        for section in cls.config:
            for key in cls.config[section]:
                attrs[cls.__GetVarName(name, section, key)] = cls.config[section][key]
                attrs[cls.__GetPropName(section, key)] = property(lambda cls: getattr(cls, cls.__GetVarName(name, section, key)))
        """
        """
                varName = cls.__GetVarName(name, section, key)
                propName = cls.__GetPropName(section, key)
                print('__SetAttributes', varName, propName)
                attrs[varName] = cls.config[section][key]
                attrs[propName] = property(lambda cls: getattr(cls, varName))
                #attrs[propName] = property(lambda cls: attrs[varName])
                #attrs[propName] = property(lambda cls: cls.config[section][key])
                #attrs[propName] = cls.config[section][key]
    ...
    def __init__(cls, name, bases, attrs):
        ...
        for section in cls.config:
            for key in cls.config[section]:
                #varName = cls.__GetVarName(name, section, key)
                #propName = cls.__GetPropName(section, key)
                setattr(cls, cls.__GetVarName(name, section, key), cls.config[section][key])
                setattr(cls, cls.__GetPropName(section, key), property(lambda cls: getattr(cls, cls.__GetVarName(name, section, key))))
                #setattr(cls, cls.__GetPropName(section, key), property(lambda cls: attrs[varName]))
                #setattr(target, propName, property(lambda target: getattr(target, varName)))
                print(cls.__GetVarName(name, section, key), getattr(cls, cls.__GetVarName(name, section, key)))
                print(cls.__GetPropName(section, key), getattr(cls, cls.__GetPropName(section, key)))
                #print(cls.__GetPropName(section, key), getattr(cls, cls.__GetPropName(section, key)), getattr(cls(), cls.__GetPropName(section, key)))
        #cls.__LoadIni()
        #cls.__SetAttributes(cls, name, attrs)
        """
        """
        ...
        """
        """
        #print('__init__', attrs['_Setting__PathDb'])
        #print('__init__', attrs['_Setting__GitRemote'])
        #print('__init__', attrs['_Setting__GithubUser'])
    ...
    def __Test(cls, target, name):
        ...
        """
                varName = cls.__GetVarName(name, section, key)
                propName = cls.__GetPropName(section, key)
                print(section, id(section), key, id(key), varName, id(varName))
                print('__SetAttributes', varName, propName)
    ...
    def __Test(cls, target, name):
        for section in cls.config:
            for key in cls.config[section]:
                #varName = cls.__GetVarName(name, section, key)
                #propName = cls.__GetPropName(section, key)
                setattr(target, cls.__GetVarName(name, section, key), cls.config[section][key])
                setattr(target, cls.__GetPropName(section, key), property(lambda target: getattr(target, cls.__GetVarName(name, section, key))))
                #setattr(cls, cls.__GetPropName(section, key), property(lambda cls: attrs[varName]))
                #setattr(target, propName, property(lambda target: getattr(target, varName)))
                print(cls.__GetVarName(name, section, key), getattr(target, cls.__GetVarName(name, section, key)))
                #print(cls.__GetPropName(section, key), getattr(target, cls.__GetPropName(section, key)))
                print(cls.__GetPropName(section, key), getattr(target, cls.__GetPropName(section, key)), getattr(target(), cls.__GetPropName(section, key)))
                #print(cls.__GetPropName(section, key), getattr(cls, cls.__GetPropName(section, key)), getattr(cls(), cls.__GetPropName(section, key)))
        #cls.__LoadIni()
        #cls.__SetAttributes(cls, name, attrs)
    """
