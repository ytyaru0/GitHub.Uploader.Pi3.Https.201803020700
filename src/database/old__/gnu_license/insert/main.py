...
#        self.__db.Licenses = dataset.connect('sqlite:///' + path_gnu_licenses_sqlite3)
                ...
                if None is not a.string:
                    ...
                    try:
                        if 'en' == self.processing_language_code:
                            self.__db_Licenses['Licenses'].insert(self.__CreateLicense(dl, dt, targetValue))
                        license = self.__db_Licenses['Licenses'].find_one(HeaderId=self.__GetHeaderId(dt))
                        if None is self.__db_Licenses['Multilingual'].find_one(LicenseId=license['Id'], LanguageCode=self.processing_language_code):
                            self.__db_Licenses['Multilingual'].insert(self.__CreateMultilingual(dt, name, self.__db_Licenses['Licenses'].find_one(HeaderId=self.__GetHeaderId(dt))['Id']))
    ...
    def __CreateLicense(self, dl, dt, targetValue):
        ...
        record = dict(
            ...
            ColorId=self.__db_Licenses['Colors'].find_one(Key=dl.get('class'))['Id'],
            Target=targetValue,
